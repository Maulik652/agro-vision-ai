...
const UNIT_LABELS    = { kg: "kg", quintal: "quintal", ton: "ton" };
const UNIT_HINTS     = {
  kg:      "Retail buyers, local vendors — min 10 kg",
  quintal: "Wholesale traders — min 1 quintal (100 kg)",
  ton:     "Bulk buyers, processors — min 1 ton (1000 kg)",
};
...
const SellCropForm = ({ defaultLocation = "Surat, Gujarat", onCreated }) => {
  ...
  const updateField = (key, value) => {
    setForm((prev) => {
      ...
      // Auto-set minOrderQty when unit changes
      if (key === "quantityUnit") {
        next.minOrderQty = String(UNIT_MIN_ORDER[value] ?? 1);
      }
      return next;
    });
  };
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const parsedMinOrder = Math.max(minOrder, Number(form.minOrderQty) || minOrder);

      if (!Number.isInteger(parsedMinOrder)) {
        throw new Error("Minimum order quantity must be a whole number.");
      }
      if (parsedMinOrder < minOrder) {
        throw new Error(`Minimum order for ${unit} must be at least ${minOrder} ${unit}.`);
      }
      if (parsedMinOrder > quantity) {
        throw new Error(`Minimum order (${parsedMinOrder}) cannot exceed total quantity (${quantity}).`);
      }
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex gap-2 sm:col-span-2 lg:col-span-2">
              <div className="flex-1">
                <input
                  type="number"
                  min={UNIT_MIN_ORDER[form.quantityUnit]}
                  step="1"
                  value={form.quantity}
                  onChange={(event) => updateField("quantity", event.target.value)}
                  placeholder={`Total qty (min ${UNIT_MIN_ORDER[form.quantityUnit]} ${form.quantityUnit})`}
                  className="w-full rounded-xl border border-[#cfe4d2] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#4c9a64]"
                  required
                />
              </div>
              <select
                ...
                className="rounded-xl border border-[#cfe4d2] bg-white px-3 py-2.5 text-sm font-semibold outline-none focus:border-[#4c9a64]"
              >
            ...
            {/* Unit hint */}
            <p className="sm:col-span-2 lg:col-span-4 -mt-1 text-xs text-[#5c7766] bg-[#f0faf2] border border-[#cfe4d2] rounded-lg px-3 py-1.5">
              📦 {UNIT_HINTS[form.quantityUnit]}
            </p>
            ...
            <input
              ...
              placeholder={`Price per ${UNIT_LABELS[form.quantityUnit]} (₹)`}
              className="rounded-xl border border-[#cfe4d2] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#4c9a64]"
              required
            />
          </div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex flex-col gap-1">
              <input
                ...
                placeholder={`Min order (${UNIT_LABELS[form.quantityUnit]})`}
                className="rounded-xl border border-[#cfe4d2] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#4c9a64]"
              />
              <p className="text-[10px] text-[#7a9a82] px-1">
                Min allowed: {UNIT_MIN_ORDER[form.quantityUnit]} {UNIT_LABELS[form.quantityUnit]}
              </p>
            </div>
            <input
              type="number"
