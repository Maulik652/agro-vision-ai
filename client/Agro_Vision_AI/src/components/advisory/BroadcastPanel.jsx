...
export default function BroadcastPanel() {
  ...
  useEffect(() => {
    ...
    socket.on("advisory_broadcast", (data) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        {/* Form */}
        <div>
          <label className="text-xs text-slate-500 block mb-1.5">Alert Title</label>
          <input value={form.title} onChange={set("title")} placeholder="e.g. Disease Outbreak Alert — Punjab"
            className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-400 transition" />
        </div>
        <div>
          ...
          <textarea value={form.message} onChange={set("message")} rows={3}
            ...
            className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-400 transition resize-none" />
        </div>
        ...
        <button onClick={() => mutation.mutate()} disabled={mutation.isPending || !form.title || !form.message}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-medium transition disabled:opacity-50">
        ...
        {alerts.length > 0 && (
          <div className="pt-3 border-t border-slate-100">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs text-slate-500 font-medium">Recent Broadcasts</p>
            </div>
              ...
              <AnimatePresence>
                {alerts.map((a) => (
                  <motion.div key={a.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    {a.message && <p className="mt-1 opacity-80 line-clamp-2">{a.message}</p>}
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
