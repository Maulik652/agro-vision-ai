    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-medium">Smart</span>
        </div>
        ...
        {open && (
          ...
          >
            <div className="p-4 space-y-3">
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <select
                      value={form[key]}
                      onChange={set(key)}
                      className="w-full bg-slate-50 border border-slate-200 text-slate-700 rounded-lg px-2.5 py-1.5 text-xs outline-none focus:border-violet-400 transition capitalize"
                    >
              ...
              <div>
                <label className="text-[10px] font-medium text-slate-400 block mb-1">Keywords (optional)</label>
                <input
                  value={form.keywords}
                  onChange={set("keywords")}
                  placeholder="e.g. rust, fungicide, early warning..."
                  className="w-full bg-slate-50 border border-slate-200 text-slate-700 placeholder-slate-400 rounded-lg px-3 py-1.5 text-xs outline-none focus:border-violet-400 transition"
                />
              </div>
              ...
              >
                {mutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Sparkles size={13} />}
              ...
              <AnimatePresence>
                {result && (
                  ...
                  >
                    {/* Title */}
                    <div className="relative p-3 bg-violet-50 border border-violet-100 rounded-xl">
                      <p className="text-[10px] font-semibold text-violet-500 mb-1">Generated Title</p>
                      <p className="text-sm font-semibold text-slate-900 pr-6 leading-snug">{result.title}</p>
                      <button onClick={() => copy(result.title, "title")} className="absolute top-2.5 right-2.5 text-violet-400 hover:text-violet-600">
                        {copied === "title" ? <Check size={12} /> : <Copy size={12} />}
                      </button>
                    </div>

                    {/* Content */}
                    <div className="relative p-3 bg-slate-50 border border-slate-100 rounded-xl">
                      <p className="text-[10px] font-semibold text-slate-400 mb-1">Generated Content</p>
                      <p className="text-xs text-slate-700 leading-relaxed line-clamp-4">{result.content}</p>
                      <button onClick={() => copy(result.content, "content")} className="absolute top-2.5 right-2.5 text-slate-400 hover:text-slate-600">
                    ...
                    {result.suggestedTitles?.length > 0 && (
                      <div>
                        <p className="text-[10px] font-semibold text-slate-400 mb-1.5">Alternative Titles</p>
                        <div className="space-y-1">
                          {result.suggestedTitles.map((t, i) => (
                            <button
                              key={i}
                              onClick={() => onApply?.({ title: t })}
                              className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-slate-50 border border-slate-100 hover:border-violet-200 hover:bg-violet-50 transition text-left"
                            >
                              <p className="text-xs text-slate-700 flex-1 leading-snug">{t}</p>
